...
# 用户一道使用简单，可重用的 JSON 格式以及我们的免费网站来共享您的科研数据。  #\n" S_RESET "\"," MSTR(
    ...
    "input domain is 2D, the initial direction can not have non-zero value in the singular dimension":  "二维仿真中发射角必须在二维平面中",
    ...
    "saved trajectory positions":  "保存的光子路径位置",
    ...
    "simulation run#":  "仿真分段#",
    "source":  "光源",
    "the specified output data format is not recognized": "指定的输出文件格式尚未支持",
    "the specified output data type is not recognized":  "指定的输出数据类型尚未支持",
