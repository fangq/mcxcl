...
//float hitgrid(float4 p[], float4 v[], float4 p0[], float *flipdir)
...
__kernel void mcx_main_loop(const int nphoton, const int ophoton,__global const uchar media[],
    ...
    // gcfg->faststep is char, use 0 as default
    // fixme:
    char faststep = 0;
    ...
    slen=len*prop.y; //unitless (minstep=grid, mus=1/grid)
//    GPUDEBUG(("p=[%f %f %f] -> <%f %f %f>*%f -> hit=[%f %f %f] flip=%d\n",p.x,p.y,p.z,v->x,v->y,v->z,len,htime.x,htime.y,htime.z,flipdir));
    ...
    p.w*=exp(-prop.x*len);
    f.x-=slen;     //remaining probability: sum(s_i*mus_i), unit-less
    f.y+=len*prop.w*gcfg->oneoverc0; //propagation time  (unit=s)
    Lmove+=len;
