...
/* Alignment macros — always needed (used by Medium, MCXParam, etc.) */
#ifdef _MSC_VER
...
#ifdef __CUDACC__
/*
 * When compiled by nvcc, CUDA provides its own vector_types.h with
 * float4, float3, int4, uint4, etc. Skip all custom type definitions
 * to avoid redeclaration errors. Only define things CUDA doesn't have.
 */

/* CUDA already defines: float4, float3, int2, int3, int4, uint2, uint3, uint4, uint */
#ifndef _UCHAR_DEFINED
    typedef unsigned char uchar;
    #define _UCHAR_DEFINED
#endif

#else /* !__CUDACC__ — original definitions for CPU/OpenCL builds */
